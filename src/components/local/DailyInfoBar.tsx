...
// Map region IDs from the local store to prayer data
// Store uses: 'qudsaya', 'dahia', 'dimas', 'all'
const prayerDataByRegion: Record<string, PrayerData> = {
  'qudsaya': qudsayaCenterPrayers,
  'dahia': qudsayaDahiaPrayers,
  'dimas': qudsayaCenterPrayers, // fallback to qudsaya center data
  'all': qudsayaCenterPrayers,   // default to qudsaya center data
};
...
// Map region IDs from the local store to weather data
const weatherDataByRegion: Record<string, WeatherData> = {
  'qudsaya': qudsayaCenterWeather,
  'dahia': qudsayaDahiaWeather,
  'dimas': qudsayaCenterWeather,
  'all': qudsayaCenterWeather,
};
...
export default function DailyInfoBar() {
  ...
  // Use the region ID string from the store to look up data
  const regionId = selectedRegion.id;
  const prayerData = prayerDataByRegion[regionId] || qudsayaCenterPrayers;
  const weatherData = weatherDataByRegion[regionId] || qudsayaCenterWeather;
        ...
        >
          ...
          <div className="flex items-stretch">
            ...
            <button
              ...
              aria-label={showDetails ? (isArabic ? 'إخفاء التفاصيل' : 'Hide details') : (isArabic ? 'عرض التفاصيل' : 'Show details')}
            >
              {showDetails
